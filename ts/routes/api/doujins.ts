...
import { API as nHentaiAPI, SortMethods } from '../../vendor/nhentai'

const router = Router()
const nhentai = new nHentaiAPI()

router.get('/doujins', (req, res, next) => {
...
router.get('/doujin/random', (_, res, next) => {
    nhentai
        .randomDoujin()
        .then((doujin) => res.json(doujin))
        .catch((err) => next(err))
})
...
router.get('/doujins/tag/:id', (req, res, next) => {
    ...
    const page = req.query.page as string
    nhentai
        .searchByTagID(id, page, sort(req.query.sort as string))
        .then((results) => res.json(results))
        .catch((err) => next(err))
})
